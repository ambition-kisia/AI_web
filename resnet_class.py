...
class SimpleModel(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(64 * 112 * 112, 8)  # Adjust this input size based on your network architecture
...
loaded_model.load_state_dict(torch.load("class_resnet.pth"))
loaded_model.eval() 
...
with torch.no_grad():
    for inputs, _ in test_dl:
        ...
        # Assuming binary classification; otherwise, use torch.softmax(outputs, dim=1)
...
for i, probs in enumerate(all_predictions):
    for class_idx, prob in enumerate(probs):
        result.append(f"{class_idx}: Probability {prob.item()*100:.2f}%")
